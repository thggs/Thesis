...
local function make_packet_connect(args)
	...
	if args.will then
		...
		assert(args.will.payload == nil or type(args.will.payload) == "string",
			"expecting will.payload to be a string or nil")
...
local function make_packet_subscribe(args)
	...
	for i, subscription in ipairs(args.subscriptions) do
		...
		if subscription.retain_as_published ~= nil then
			assert(type(subscription.retain_as_published) == "boolean",
				"expecting .subscriptions[" .. i .. "].retain_as_published to be a boolean")
		end
		if subscription.retain_handling ~= nil then
			assert(type(subscription.retain_handling) == "number",
				"expecting .subscriptions[" .. i .. "].retain_handling to be a number")
			assert(check_retain_handling(subscription.retain_handling),
				"expecting .subscriptions[" .. i .. "].retain_handling to be a valid Retain Handling option")
		end
...
local function parse_properties(ptype, read_data, input, packet)
	...
	local uprop_id = properties.user_property
	local allowed = assert(allowed_properties[ptype],
		"no allowed properties for specified packet type: " .. tostring(ptype))
	...
	while input[1] < props_end do
		...
		if not allowed[prop_id] then
			if not allowed[prop_id] then
				return false,
					"property " .. tostring(properties[prop_id]) ..
					" (" .. prop_id .. ") is not allowed for that packet type"
			end
		end
...
local function parse_packet_publish(ptype, flags, input)
	...
	-- DOC: 3.3.2.3 PUBLISH Properties
	local packet = setmetatable({ type = ptype, dup = dup, qos = qos, retain = retain, packet_id = packet_id, topic =
	topic }, packet_mt)
...
local function parse_packet_puback(ptype, flags, input)
	...
	local packet = setmetatable({ type = ptype, packet_id = packet_id, rc = 0, properties = {}, user_properties = {} },
		packet_mt)
	if input.available > 0 then
