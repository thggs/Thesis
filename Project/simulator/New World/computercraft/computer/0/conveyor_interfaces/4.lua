...
local function openDepot()
    rs.setBundledOutput(side, colors.subtract(rs.getBundledOutput(side), colors.red))
end
...
local function start()
    rs.setBundledOutput(side, colors.subtract(rs.getBundledOutput(side), colors.yellow))
end

local function stop()
    rs.setBundledOutput(side, colors.combine(rs.getBundledOutput(side), colors.yellow))
end
...
local function getSensor()
    local bool = rs.testBundledInput(side, colors.combine(colors.lightGray))
    return bool
end
