...
The implementation needs to start with the deployment agents, since these are responsible for launching all other agent types. The \acrfull{DA} is tasked with deploying \acrfullpl{RA} and \acrfullpl{TA}. The \acrshort{DA} needs a \acrfull{GUI} to allow a user to launch these agents with the right configurations. These must include the agent type to be launched, the type of \acrlong{LL} the agent must use and its configurations. In addition to the \acrshort{DA}, the \acrfull{PM} also needs to launch agents, more specifically, \acrfullpl{PA}. This should also be done through a \acrshort{GUI} but this one should only show buttons per product type, and some information on the already deployed \acrshortpl{PA}.\\
...
The "xmlMarketplace" is loaded during "setup" with the method "getMarketplaceLibraries", using the file pointed to by the "marketplaceXMLPath" String. It holds the path of the \acrshort{XML} file that contains the available \acrlongpl{LL}.
...
\begin{figure}[h!]
	\centering
	\includegraphics[scale=0.75]{Const_Class_Diagram}
...
In order to use these \acrlongpl{LL} however, the hardware that would work underneath the agent would also need to implement these three protocols in some way. In addition, a configuration file must always be provided to the \acrshort{LL} for it to communicate properly.\\
...
The \acrshort{LL} will now forward the skill through the protocol it is implementing, in whatever format it was designed for. A developer could reformat this message to whatever type they want, but the \acrshort{LL} must always return a result of type String. As long as these rules are obeyed, \acrlongpl{LL} can be used in the way most suitable to the system they are in. The result is passed to the \acrshort{ME} as a String, which is then passed to the agent.\\
...
\begin{figure}[h!]
	\centering
	\includegraphics{MAS_Operations_2}
...
\begin{figure}[h!]
	\centering
	\includegraphics{MAS_Operations_4}
