...
%As we can see from these works, one of the main aspects of retrofitting hardware into a \gls{CPPS} is the interface between the physical and virtual environment. A good interface does a lot of the heavy-lifting, by permitting real-time communications between the two layers of the \gls{CPPS}.

%\begin{table}[h!]
%	\centering
%	\caption{\gls{CPPS}s Key Challenges. Source: Adapted from \cite{LEITAO201611}}
%	\begin{tabular}{l|l|l|l}
%		\toprule
%		Area & Challenge & Difficulty & Priority\\
%		\hline
%		\multirow{6}{*}{\gls{CPS} Capabilities}	& Real-time control of \gls{CPS} systems          										& High   & High   \\ \cline{2-4}
%						      					& Real-time \gls{CPS} \gls{SoS}                  								 		& High   & Medium \\ \cline{2-4}
%						      					& Optimization in \gls{CPS} and their application									 	& High   & Medium \\ \cline{2-4}
%						      					& On-\gls{CPS} advanced analytics                 										& Medium & High   \\ \cline{2-4}
%						      					& Modularization and servification of \gls{CPS}  										& Low    & High   \\ \cline{2-4} 
%						      					& Energy efficient \gls{CPS}                      										& Medium & Medium \\ \cline{2-4}
%		\hline
%		\multirow{3}{*}{CPS Management}			& Lifecycle management of \gls{CPS}                                  					& Medium & Medium \\ \cline{2-4}
%					   			& Management of (very) large scale \gls{CPS} and \gls{CPS}-\gls{SoS} 					& High   & High   \\ \cline{2-4}
%		               			& Security and trust management for heterogeneous \gls{CPS}          					& High   & High   \\ \cline{2-4}
%		\hline
%		\multirow{5}{*}{CPS Engineering} 		& Safe programming and validation of \gls{CPS} \gls{SoS}                                & High   & High \\ \cline{2-4}
%								& Resilient risk-mitigating \gls{CPS}                                       	        & High   & High \\ \cline{2-4}
%								& Methods and tools for \gls{CPS} lifecycle support                                     & High   & High \\ \cline{2-4}
%								& New operating systems and programming languages for \gls{CPS} and \gls{CPS} \gls{SoS} & Medium & Low  \\ \cline{2-4}
%								& Simulation of CPS and of \gls{CPS}-\gls{SoS}                                          & Medium & High \\ \cline{2-4}
%		\hline
%		\multirow{5}{*}{CPS Infrastructures} 	& Interoperable \gls{CPS} services                                                    	& Medium & High   \\ \cline{2-4}
%												& Migration solutions to emerging \gls{CPS} infrastructures                          	& Medium & High   \\ \cline{2-4}
%												& Integration of heterogeneous/mobile \\ hardware and software technologies in \gls{CPS} 	& Low    & Medium \\ \cline{2-4}
%												& Provision of ubiquitous \gls{CPS} services                                         	& Medium & Medium \\ \cline{2-4}
%												& Economic impact of \gls{CPS} Infrastructure                                        	& High   & High   \\ \cline{2-4}
%		\hline
%		\multirow{3}{*}{CPS Ecosystems} 			& Emergent behavior of \gls{CPS}    													& High   & Medium \\ \cline{2-4}
%			          			& \gls{CPS} with humans in the loop 													& High   & High   \\ \cline{2-4}
%		               			& Collaborative \gls{CPS}          														& Medium & Medium \\ \cline{2-4}
%		\hline
%		\multirow{4}{*}{CPS Information Systems} & Artificial intelligence in \gls{CPS} 										 	     	& High   & High   \\ \cline{2-4}
%								& Cross-domain large-scale information integration to \gls{CPS} infrastructures	     	& Medium & Low    \\ \cline{2-4}
%								& Transformation of \gls{CPS} data and information analytics to actionable knowledge 	& High   & High   \\ \cline{2-4}
%								& Knowledge-driven decision making/management 								         	& High   & Medium \\ \cline{2-4}
%		\bottomrule
%	\end{tabular}
%	\label{tab:cpps_challenges}
%\end{table}
